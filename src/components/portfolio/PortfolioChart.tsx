      ...
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          ...
          <div className="flex space-x-1 bg-muted p-1 rounded-lg">
            {timeframes.map(timeframe => <button key={timeframe} onClick={() => setSelectedTimeframe(timeframe)} className={`px-3 py-1 text-sm rounded-md transition-colors ${selectedTimeframe === timeframe ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground'}`}>
                {timeframe}
              </button>)}
          </div>
        </div>
        ...
        <div className="h-80 w-full">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={chartData}>
              ...
              <XAxis dataKey="displayDate" axisLine={true} tickLine={true} tick={{
              fontSize: 12,
              fill: '#6b7280'
            }} stroke="#6b7280" />
              <YAxis domain={['dataMin - 1000', 'dataMax + 1000']} axisLine={true} tickLine={true} tick={{
              fontSize: 12,
              fill: '#6b7280'
            }} tickFormatter={value => `£${(value / 1000).toFixed(0)}k`} stroke="#6b7280" />
